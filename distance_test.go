...
type CreateNaiveHomologyMatrixTest struct {
	//input:
	// sequences []string
	sequences	Sequences
...
//CreateNaiveHomologyMatrix() test:
//Input: TestCreateNaiveHomologyMatrix() takes nothing as input besides a command.
//Output: Tells you if your function has passed all of the CreateNaiveHomologyMatrix test cases.
func TestCreateNaiveHomologyMatrix(t *testing.T) {
	//Reading inputs and resulting outputs of CreateNaiveHomologyMatrix test cases.
	tests := ReadCreateNaiveHomologyMatrixTest("Tests/CreateNaiveHomologyMatrix/")
	//Entering the inputs from the test cases into our function and making sure the outputs of this function are 
	//equal to the outputs of the test cases.
	for _, test := range tests {
		functionAnswer := CreateNaiveHomologyMatrix(test.sequences)

		for r := 0; r < len(functionAnswer); r ++ {
			for c := 0; c < len(functionAnswer[r]); c ++ {
				if roundFloat(functionAnswer[r][c], 2) != test.distanceMatrix[r][c] {
					t.Errorf("Failed test CreateNaiveHomologyMatrix")
				}
			}
		}
	}
...
//Input: CreateNaiveHomologyMatrixTest() takes as input a directory of strings ("Test/CreateNaiveHomologyMatrix/")
//Output: Returns an array of []CreateNaiveHomologyMatrixTest.
func ReadCreateNaiveHomologyMatrixTest(directory string) []CreateNaiveHomologyMatrixTest {
	//Reading in the input files
	inputFiles := ReadDirectory(directory + "/Inputs")
	numFiles := len(inputFiles)
	tests := make([]CreateNaiveHomologyMatrixTest, numFiles)
	...
	for i, inputFile := range inputFiles {
		...
		sequences := make(Sequences, len(inputs))
		for s := 0; s < len(inputs); s ++ {
			sequences[s].sequence = inputs[s]
		}
	...
	//Assigning each output value in the test case to the correct field of the corresponding CreateNaiveHomologyMatrixTest for
	//each CreateNaiveHomologyMatrixTest object within the array.
	for i, outputFile := range outputFiles {
		//Create a list with each indices designated as the particular field of the CreateNaiveHomologyMatrixTest
		lines := ReadOutput(directory + "Outputs/" + outputFile.Name())
